...
<body background="./../../../img/backgrounds/aqua.jpg">  
 <form class="form-horizontal well" method="post" name="contact" action="listar_ventasFechas.php" onSubmit='return Validar(this);' style="border-right-width: 100px; margin-left: 0px; border-top-width: 100px; border-left-width: 110px;">
	...
	<p><span class="label label-primary" for="input02">Buscar Ventas con fecha creacion entre</span> <input readonly="readonly" type="text" id="fecha_inicio1" name="fecha_inicio1" size="20" value="" onBlur="return validar(event);"> <span class="label label-primary" for="input02"> Y</span>  <input readonly="readonly" type="text" id="fecha_inicio2" name="fecha_inicio2" size="20" value="" onBlur="return Validar(event);">
        ...
        <th>Pendiente</th>
                    ...
                    <tbody>
					 ...
					 $result = $base_datos->sql_query($venta->listarVentasNaturales());
			...
			echo "<td>";	
			echo "<span class='label label-default' for='input02'>Natural<span>";  		    
			echo "</td>";	
	 ...
	 ?>
       <?php 
					 ...
					 $result = $base_datos->sql_query($venta->listarVentasJuridicos());
						 while($row = $base_datos->sql_fetch_assoc($result)) {
			...
			echo "<td>";	
			echo "<span class='label label-warning' for='input02'>Juridico<span>";  		    
			echo "</td>";	
			echo "<td>".$nombre_sucursal."</td>";	  		    
			//echo "<td>";
			...
			if ($pagado == 'pagado') {
			echo "<td>";	
			echo "<span class='label label-sucess' for='input02'>Pagado<span>";  		    
			echo "</td>";	
}  else {
   			echo "<td>";	
			echo "<span class='label label-danger' for='input02'>Pendiente<span>";  		    
			...
			echo "</td>";	
   			...
   			$precioCuotas = sprintf( number_format("$monto_pago / $cuota", 0, '.', '.'));
			echo "<td>";	
			echo "<span class='label label-danger' for='input02'>$$precioCuotas por cuotas<span>";  		    
			echo "</td>";
}	  		    
...
<?php 
 $result = $base_datos->sql_query($venta->sumarVentas());
				 while($row = $base_datos->sql_fetch_assoc($result)) {
			     $total 			= $row['total'];
	        ...
	        echo "<tr class = 'light'>";
			 echo "<td colspan='11'>Cantidad total de ventas</td>";
			echo "<td>".$total."</td>";	  		    
