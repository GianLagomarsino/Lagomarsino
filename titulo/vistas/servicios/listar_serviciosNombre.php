...
<body background="./../../../img/backgrounds/aqua.jpg">
 <form class="form-horizontal well" method="post" name="contact" action="listar_serviciosNombre.php?nombre_servicio=<?php $nombre_buscado; ?>" onSubmit='return Validar(this);' style="border-right-width: 100px; margin-left: 0px; border-top-width: 100px; border-left-width: 100px;">
		 ...
		 <p><span class="label label-primary" for="input02">Buscar Por Nombre Servicio</span> <input type="text" name="nombre_buscado" size="20" value="">
		...
		<?php 
					$servicio = new servicio();
					$nombre_servicio = $_POST['nombre_buscado'];
		  	if(isset($nombre_servicio)){
				$nombre_buscado = $nombre_servicio;
				}
				else{
				$nombre_buscado = "";
				}
		  ?>	
					 ...
					 $result = $base_datos->sql_query($servicio->listarServiciosName($nombre_buscado));
		    ...
		    echo "<th><a href='ingresar_ventas.php?servicio_id=".$servicio_id."'><img src='../../images/pagar.png'></th>";
...
<?php 
					 ...
					 $result = $base_datos->sql_query($servicio->contarServiciosNombre($nombre_buscado));
						 while($row = $base_datos->sql_fetch_assoc($result)) {
					  ...
					  $number 			= $row['number'];
								...
								echo "<td colspan='9'>Cantidad total de servicios activos en relacion al rango buscado</td>";
